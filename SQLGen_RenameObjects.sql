...
DESCRIPTION:
--------------------------------------------------------------------------------
Generate a script used to perform a OBJECT RENAME command for all objects
...
SELECT 'execute dbo.sp_rename @objname = ''dbo.'+[name]+''', @newname = ''OLDNAMEHERE_'+[name]+''', @objtype = ''OBJECT'''
FROM   [sys].[objects]
WHERE  [type] IN('U', 'P')
ORDER BY [name];