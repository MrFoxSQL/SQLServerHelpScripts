...
DESCRIPTION:
--------------------------------------------------------------------------------
Generate a script used to recreate the SQL Server custom system messages
...
SELECT 'execute master.dbo.sp_addmessage '+'@msgnum = '+CONVERT( VARCHAR(50), [error])+', @severity  = '+CONVERT(VARCHAR(50), [severity])+', @msgtext = '''+REPLACE([description], '''', '''''')+''''+', @lang = ''us_english'''+', @replace = ''replace'''
FROM   [sysmessages]
WHERE  [error] > 50000;