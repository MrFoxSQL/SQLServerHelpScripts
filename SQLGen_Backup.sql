...
DESCRIPTION:
--------------------------------------------------------------------------------
Generate a script used to perform a database BACKUP command 
...
SELECT 'PRINT ''['+[name]+'] : '' + convert(varchar(25), getdate(), 121)'+CHAR(13)+CHAR(10)+'GO'+CHAR(13)+CHAR(10)+'BACKUP DATABASE ['+[name]+'] TO DISK = ''X:\BACKUP\SQLDataDumps\'+[name]+'_PostMigration.BAK'' WITH COMPRESSION, INIT, STATS = 10'+CHAR(13)+CHAR(10)+'GO'+CHAR(13)+CHAR(10)
FROM   [master].[dbo].[sysdatabases]
WHERE  [name] NOT IN('tempdb')
ORDER BY [name];
GO

SELECT 'BACKUP LOG ['+[name]+'] TO DISK = ''X:\BACKUP\SQLLogDumps\'+[name]+'.trn'' WITH INIT, STATS = 10'
FROM   [master].[dbo].[sysdatabases]
ORDER BY [name];
GO